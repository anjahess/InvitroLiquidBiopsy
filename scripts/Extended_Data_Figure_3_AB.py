...
results_file = "./sourcedata/FIG_3EXT/E3A-D/cpg_df.csv"
plot_dir = "./sourcedata/FIG_3EXT/E3A-D/"
...
g = sns.catplot(
    ...
    x=x, y=y, orient="h", col=col_val,
    kind="bar", sharex=False, margin_titles=True,
...
####################################################################################
# Iterate through directory, collect data per file
####################################################################################
path = "./sourcedata/FIG_3EXT/E3A-B/"
...
####################################################################################
# Get data for the read histogram dataframe (size 0-20K bp)
####################################################################################
frag_df_path = f"{path}fragment_df.csv"
cov_df_path = f"{path}cov_df.csv"
...
plt.savefig(f"{path}ont_mean_read_coverage_pass.pdf", bbox_inches='tight')
plt.close()


####################################################################################
# 1. Plot the fragment length data
####################################################################################
...
plt.xlim(0, None)
sns.despine()
plt.savefig(f"{path}bp_sizes.pdf", dpi=50)
